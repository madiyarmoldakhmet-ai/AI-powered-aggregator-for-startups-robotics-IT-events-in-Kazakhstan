...
- `SEED_MOCK_EVENTS`: только для локальной демонстрации; по умолчанию `false`, чтобы тестовые события не выдавались пользователям.
...
7. Не включать `SEED_MOCK_EVENTS` в production; перейти на PostgreSQL и применять только `alembic upgrade head`.
...
- API не выполняет интерактивную авторизацию Telethon: сначала нужно один раз создать авторизованную сессию отдельной командой, затем scheduler сможет читать каналы без блокировки Uvicorn.
...
- Старые mock-события, созданные в БД до отключения seed, нужно удалить вручную по `source='mock'` или пересоздать локальную БД.
...
- События без даты (`Не указана`) намеренно не сохраняются ingest-слоем: это предотвращает выдачу неподтверждённых или псевдособытий из fallback-режима.
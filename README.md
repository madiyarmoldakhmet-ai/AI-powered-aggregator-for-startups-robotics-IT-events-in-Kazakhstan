...
API-сервис запускает APScheduler и проверяет источники каждые 3 часа. Список публичных Telegram-каналов задаётся через `TELEGRAM_CHANNELS` (например, `astanahub,techgarden,jetisudigital`), а открытые страницы агрегаторов и web-версии пабликов через `SCRAPER_URLS` (URL разделяются запятыми). Для Telethon нужны `TELEGRAM_API_ID` и `TELEGRAM_API_HASH`; user-сессия `eventscout` создаётся при первом подключении.

Команда `/subscribe` сохраняет chat ID в БД. При появлении нового события сервис отправляет подписчикам заголовок, дату, город и ссылку. Instagram и TikTok используются только через публичные HTML-страницы или агрегаторы, без обхода авторизации и ограничений платформ.
